...
(function(){
 ...
 async function openPanel(){
   ...
   p.innerHTML='<div class="street"><b>'+(ctx.shared?'👥 Moulet-Marcenat — tournée commune':'👥 '+esc(teams.find(t=>t.id===ctx.teamId)?.name||'Équipe'))+'</b>'+
    ...
    '<label>Avec qui as-tu fait les passages ?</label><select id="dayPartner">'+opts+'</select>'+
    '<div class="muted" style="margin:7px 0">Si la personne n’a pas de compte dans l’application :</div>'+
    '<input id="dayPartnerFree" placeholder="Nom et prénom de l’ancien / accompagnant"><br><br>'+
